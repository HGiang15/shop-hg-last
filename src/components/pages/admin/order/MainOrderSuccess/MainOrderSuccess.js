...
import styles from './MainOrderSuccess.module.scss';
...
const MainOrderSuccess = () => {
	...
	const handleConfirmDelete = async () => {
		try {
			...
			toast.error(error.message || 'Xoá đơn hàng thất bại');
		}
	};
										...
										<>
											<IconCustom
												...
												onClick={() => {
													...
													setSelectedOrderCode(order.product?.items?.[0]?.name);
													setIsModalOpen(true);
												}}
											/>
											<IconCustom
...
export default MainOrderSuccess;
